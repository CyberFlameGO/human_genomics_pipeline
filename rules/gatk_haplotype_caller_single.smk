rule gatk4_HaplotypeCaller_single:
    ...
    shell:
        "(time gatk HaplotypeCaller -I {input.bams} -R {input.refgenome} -D {input.dbsnp} -O {output} --tmp-dir {params.tdir} --native-pair-hmm-threads {threads} {params.padding} {params.intervals}) 2> times/gatk4_HaplotypeCaller_single_time.txt"